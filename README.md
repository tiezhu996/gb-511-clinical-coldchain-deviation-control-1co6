...
- 温控规则草稿生效前强制影响核验：同产品类别和场站仍存在 `in_transit` 容器或未结束偏差时整次生效失败（422 `activation_blocked`）并返回全部编号；草稿状态/版本与旧规则均不变，阻断原因与影响清单持久化在草稿上，处理完毕后用同一版本再次提交才生效，同范围旧规则在同一事务中转为 `superseded`。
