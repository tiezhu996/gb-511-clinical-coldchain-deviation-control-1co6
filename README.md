...
- 温控规则草稿生效前执行影响核验：同一产品类别和场站仍有在途容器（`in_transit`）或未结束偏差（`open/in_review/decided`）时整次生效被拒绝，返回全部命中编号并把阻断原因持久化到草稿；草稿状态、版本与旧规则均不变，处理完毕后再次提交才生效，同作用域旧规则在同一事务内转为 `superseded`，回读规则或审计历史仍能看到上次阻断原因。
